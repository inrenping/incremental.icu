...
export function AppCard({ app, onConnect, onRefresh }: AppCardProps) {
  ...
  return (
    <Card className="relative">
      {app.isConnected && app.total_count !== undefined && app.total_count > 0 && (
        <Link href={`/dash/activities?platform=${app.platform}&email=${app.email}`} className="absolute -top-3 -right-3 z-20 hover:scale-110 transition-transform">
